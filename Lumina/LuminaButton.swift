...
//
//  Based on a project by David Okun.
...
final class LuminaButton: UIButton
{
    ...
    init(with systemStyle: SystemButtonType)
    {
        ...
        {
        case .torch:
            self.image = UIImage(named: "cameraTorchOff",
                                 in: Bundle(for: LuminaViewController.self),
                                 compatibleWith: nil)
            self.frame = CGRect(origin: CGPoint(x: 10, y: 10),
                                size: CGSize(width: self.squareSystemButtonHeight,
                                             height: self.squareSystemButtonHeight))
            addButtonShadowEffects()
        case .cameraSwitch:
            self.image = UIImage(named: "cameraSwitch",
                                 in: Bundle(for: LuminaViewController.self),
                                 compatibleWith: nil)
            self.frame = CGRect(origin: CGPoint(x: UIScreen.main.bounds.maxX - 50,
                                                y: 10),
                                size: CGSize(width: self.squareSystemButtonWidth,
                                             height: self.squareSystemButtonHeight))
            addButtonShadowEffects()
        case .cancel:
            self.text = "X"
            self.frame = CGRect(origin: CGPoint(x: 10,
                                                y: UIScreen.main.bounds.maxY - 50),
                                size: CGSize(width: self.cancelButtonWidth,
        ...
        case .shutter:
            ...
            minY -= 80
            self.frame = CGRect(origin: CGPoint(x: UIScreen.main.bounds.midX - 35,
                                                y: minY),
                                size: CGSize(width: self.shutterButtonDimension,
                                             height: self.shutterButtonDimension))
...
extension LuminaButton
{
    func updateTorchIcon(to state: SystemButtonType.FlashState)
    {
        ...
        {
        case .on:
            self.image = UIImage(named: "cameraTorchOn",
                                 in: Bundle(for: LuminaViewController.self),
                                 compatibleWith: nil)
        ...
        case .auto:
            self.image = UIImage(named: "cameraTorchAuto",
                                 in: Bundle(for: LuminaViewController.self),
                                 compatibleWith: nil)
