...
extension LuminaCamera
{
    func getNewVideoInputDevice() -> AVCaptureDeviceInput?
    {
        do
        {
            guard let device = getDevice(with: self.position == .front ? AVCaptureDevice.Position.front
    ...
    func purgeVideoDevices()
    {
        ...
        {
            if oldOutput == self.videoDataOutput
               || oldOutput == self.photoOutput
               || oldOutput == self.metadataOutput
               || oldOutput == self.videoFileOutput
            {
    ...
    {
        guard let device = self.currentCaptureDevice else { return }
        ...
        for vFormat in device.formats
        {
            ...
            guard let frameRate = ranges.first else { continue }
    ...
    func updateZoom()
    {
        ...
        {
            try device.lockForConfiguration()
            let newZoomScale = min(maxZoomScale,
                                   max(Float(1.0), min(currentZoomScale,
                                                       Float(device.activeFormat.videoMaxZoomFactor))))
