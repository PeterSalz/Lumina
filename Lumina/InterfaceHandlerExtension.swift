...
extension LuminaViewController
{
    ...
    {
        guard self.position == .back else { return }
        
        currentZoomScale = min(maxZoomScale,
                               max(1.0, beginZoomScale * Float(recognizer.scale)))
    ...
    {
        self.cancelButton.center = CGPoint(x: self.view.frame.minX + 55,
                                           y: self.view.frame.maxY - 45)
        ...
        if self.view.frame.width > self.view.frame.height
        {
            ...
            self.shutterButton.center = CGPoint(x: maxX - 45,
        ...
        else
        {
            ...
            self.shutterButton.center = CGPoint(x: self.view.frame.midX,
                                                y: maxY - 45)
        }
        
        self.switchButton.center = CGPoint(x: self.view.frame.maxX - 25,
                                           y: self.view.frame.minY + 25)
        self.torchButton.center = CGPoint(x: self.view.frame.minX + 25,
                                          y: self.view.frame.minY + 25)
        /*
            Use more width, if text has been moved down below the buttons
            (e.g. notch on iPhone X):
        */
        let textWidth = self.view.frame.maxX - (minY > 35 ? 20 : 110)
        
        self.textPromptView.frame.size = CGSize(width: textWidth,
                                                height: 80)
        self.textPromptView.layoutSubviews()
        self.textPromptView.center = CGPoint(x: self.view.frame.midX,
                                             y: minY + 45)
    }
    ...
    func handleCameraSetupResult(_ result: CameraSetupResult)
    {
        ...
        DispatchQueue.main.async
        {
            ...
            case .unknownError:
                self.textPrompt = "Unkown error occurred while loading Lumina - please try again."
