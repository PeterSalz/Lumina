...
extension LuminaCamera: AVCapturePhotoCaptureDelegate
{
                     ...
                     error: Error?)
    {
        ...
        guard let image = photo.normalizedImage(forCameraPosition: self.position)
        else { return }
        ...
        photoCollectionQueue.sync
        {
            ...
            {
                guard var collection = self.currentPhotoCollection
                else { return }
                     ...
                     error: Error?)
    {
        ...
        else
        {
            ...
            guard let image = buffer.normalizedStillImage(forCameraPosition: self.position)
            else { return }
            
            delegate?.stillImageCaptured(camera: self,
                                         image: image,
                                         livePhotoURL: nil,
                                         depthData: nil)
        }
