...
extension LuminaCamera
{
    func requestVideoPermissions()
    {
        ...
        {   success in
            ...
            if success
            {
                ...
                self.delegate?.cameraSetupCompleted(camera: self,
                                                    result: .requiresUpdate)
            }
            else
            {
    ...
    func updateAudio(_ completion: @escaping (_ result: CameraSetupResult) -> Void)
    {
        self.sessionQueue.async
        {
            ...
            {
            case .authorized:
                guard let audioInput = self.getNewAudioInputDevice()
                else
                {
                ...
                guard self.session.canAddInput(audioInput)
                else
                {
                    completion(CameraSetupResult.invalidAudioInput)
                    return
    ...
    private func videoSetupApproved() -> CameraSetupResult
    {
        self.torchState = .off
        /*
            Set here so that device inmput can be added to session.
            Resolution can be checked for update later.
        */
        self.session.sessionPreset = .high
        
        guard let videoInput = self.getNewVideoInputDevice()
        else { return .invalidVideoInput }
    ...
    private func checkSessionValidity(for input: AVCaptureDeviceInput) -> CameraSetupResult?
    {
        ...
        guard self.session.canAddOutput(self.videoDataOutput)
        else
        {
        ...
        guard self.session.canAddOutput(self.metadataOutput)
        else
        {
        ...
        {
            guard self.session.canAddOutput(self.videoFileOutput)
            else
            {
        ...
        {
            guard self.session.canAddOutput(depthDataOutput)
            else
            {
                Log.error("cannot add depth data output with thi settings map")
