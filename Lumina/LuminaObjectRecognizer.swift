...
public struct LuminaPrediction
{
    ...
    /// The unique identifier associated with this prediction, as determined by the Vison framework.
    public var UUID: UUID
}

/**
    An object that represents a collection of predictions that Lumina detects, along
...
final class LuminaObjectRecognizer: NSObject
{
    ...
    func recognize(from image: UIImage,
                   completion: @escaping ([LuminaRecognitionResult]?) -> Void)
    {
        guard let coreImage = image.cgImage
        else
        {
            completion(nil)
            return
        ...
        for modelPair in modelPairs
        {
            ...
            guard let visionModel = try? VNCoreMLModel(for: modelPair.0)
            else
            {
            ...
            {   request, error in
                ...
                {
                    recognitionResults.append(LuminaRecognitionResult(predictions: nil,
                                                                      type: modelPair.1))
                    recognitionGroup.leave()
                }
                else if let results = request.results
                {
                    ...
                    recognitionResults.append(LuminaRecognitionResult(predictions: mappedResults,
                                                                      type: modelPair.1))
                    recognitionGroup.leave()
                }
            }
    ...
    private func mapResults(_ objects: [Any]) -> [LuminaPrediction]
    {
        ...
        for object in objects
        {
            ...
            {
                results.append(LuminaPrediction(name: object.identifier,
