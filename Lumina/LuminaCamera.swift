...
{
    func stillImageCaptured(camera: LuminaCamera,
                            image: UIImage,
                            livePhotoURL: URL?,
                            depthData: Any?)
    
    func videoFrameCaptured(camera: LuminaCamera,
                            frame: UIImage)
    
    @available(iOS 11.0, *)
    func videoFrameCaptured(camera: LuminaCamera,
                            frame: UIImage,
                            predictedObjects: [LuminaRecognitionResult]?)
    
    func depthDataCaptured(camera: LuminaCamera,
                           depthData: Any)
    
    func videoRecordingCaptured(camera: LuminaCamera,
                                videoURL: URL)
    
    func finishedFocus(camera: LuminaCamera)
    
    func detected(camera: LuminaCamera,
                  metadata: [Any])
    
    func cameraSetupCompleted(camera: LuminaCamera,
                              result: CameraSetupResult)
...
final class LuminaCamera: NSObject
{
    ...
    var videoBufferQueue = DispatchQueue(label: "com.petersalz.Lumina.videoBufferQueue",
                                         attributes: .concurrent)
    var metadataBufferQueue = DispatchQueue(label: "com.petersalz.Lumina.metadataBufferQueue")
    var recognitionBufferQueue = DispatchQueue(label: "com.petersalz.Lumina.recognitionBufferQueue")
    var sessionQueue = DispatchQueue(label: "com.petersalz.Lumina.sessionQueue")
    var photoCollectionQueue = DispatchQueue(label: "com.petersalz.Lumina.photoCollectionQueue")
    var depthDataQueue = DispatchQueue(label: "com.petersalz.Lumina.depthDataQueue")
