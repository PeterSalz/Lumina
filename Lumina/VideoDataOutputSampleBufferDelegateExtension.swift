...
extension LuminaCamera: AVCaptureVideoDataOutputSampleBufferDelegate
{
    ...
    {
        guard let image = sampleBuffer.normalizedVideoFrame()
        else { return }
        ...
        if #available(iOS 11.0, *)
        {
            if let modelPairs = self.streamingModels
            {
                ...
                guard let recognizer = self.recognizer as? LuminaObjectRecognizer
                else
                {
                ...
                recognizer.recognize(from: image, completion:
                {   results in
                    DispatchQueue.main.async
                    {
                        self.delegate?.videoFrameCaptured(camera: self,
                                                          frame: image,
                                                          predictedObjects: results)
                    }
                })
            }
            else
